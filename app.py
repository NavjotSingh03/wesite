...
def find_all(): #Afficher tout les pays 

	countries_info = response("/all/")
	return countries_info


def find_by_name(name): #Trouver les informations d'un pays en fonction de son nom
	countries_info = response("/name/" + name)
	return countries_info

#nam = find_by_name('france')
#print(nam)

def find_lang(language): #Trouver un pays en fonction d'une langue 
...
def find_by_capital(capital_name): #Trouver un pays en fonction de sa capital
	
	countries_info = response("/capital/" + capital_name)
	return countries_info

# capit=find_by_capital('paris')
# print(capit)

#def find_all_capitals(all_capitals_name): #Trouver un pays en fonction de sa capital
	#countries_info = response("/capital/")
	#return countries_info

def find_by_region(region_name): #Chercher les pays en fonction des continents

	countries_info = response("/region/" + region_name)
	return countries_info

# region = find_by_region('europe')
# print(region)

def find_by_currency(currency): #Chercher les pays en fonction de la monnaie locale

	countries_info = response("/currency/" + currency)
	return countries_info

# curr = find_by_currency('eur')
# print(curr)

def find_by_callingcode(code_num): #Numéro unique pour chaque pays. Constitué de 2 à 4 chiffres

	countries_info = response("/callingcode/" + code_num)
	return countries_info


@app.errorhandler(404) #Gérer l'erreur 404
...
# Formulaire :
class myForm(Form):
    pays = StringField("Saisissez le nom  d'un pays :", validators=[InputRequired()])
...
def pays():
    ...
    return render_template('pays2.html', title=pays, name=name,  pays=pays, monnaies=monnaies, region=region, languages=languages, drapeaux=drapeaux, habitants=habitants, pays_name_form=pays_name_form, pays_name_api=pays_name_api)
